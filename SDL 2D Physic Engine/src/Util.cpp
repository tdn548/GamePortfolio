...
float Util::Angle360(glm::vec2 from, glm::vec2 to)
{
	if (from.x > to.x)
	{
		if (from.y > to.y)
		{
			float param2 = (from.x - to.x) / (from.y - to.y);
			return 270 + (atan(param2) * (180 / 3.1416));
		}
		else
		{
			float param = (to.y - from.y) / (from.x - to.x);
			return (atan(param) * (180 / (3.1416)));
		}
	}
	else
	{
		if (from.y > to.y)
		{
			float param3 = (from.y - to.y) / (to.x - from.x);
			return 180 + (atan(param3) * (180 / (3.1416)));
		}
		else
		{
			float param4 = (to.x - from.x) / (to.y - from.y);
			return 90 + (atan(param4) * (180 / (3.1416)));
		}
	}
}
...
glm::vec2 Util::AngleMagnitudToVector(float angleDegrees, float magnitud)
{
	glm::vec2 Magnitud;
	Magnitud.x = cos(-angleDegrees * Util::Deg2Rad) * magnitud;
	Magnitud.y = sin(-angleDegrees * Util::Deg2Rad) * magnitud;

	if (angleDegrees == 90 || angleDegrees == -90 || angleDegrees == 270 || angleDegrees == -270)
	{
		Magnitud.x = 0;
	}

	if (angleDegrees == 180 || angleDegrees == 0 || angleDegrees == -180 || angleDegrees == -0 || angleDegrees == 360)
	{
		Magnitud.y = 0;
	}
